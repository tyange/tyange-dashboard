...
export default function NotificationsPage() {
  ...
  return (
    <section class="space-y-8 pb-10" aria-label="구독">
      ...
      <BrowserAlertsSettingsSection title="현재 브라우저 알림" showTechnicalDetails />

      <section class="pt-8">
        <div class="mb-5 flex items-end justify-between gap-4">
          <h2 class="text-2xl font-semibold tracking-tight text-foreground">목록</h2>
          <button type="button" class={ghostButton} onClick={() => void loadRssSources()} disabled={rssLoading()}>
            새로고침
          </button>
        </div>
