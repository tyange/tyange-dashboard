import type { BudgetWeeksResponse, ConsumptionRecordsResponse, WeeklySummary } from './types'
...
export async function fetchConsumptionRecords(weekKey: string): Promise<ConsumptionRecordsResponse> {
  const response = await fetch(`${apiBaseUrl}/budget/records/${weekKey}`, {
    headers: { Accept: 'application/json' },
  })
